...
print("The softmax action function was used for the output layer because it converts the output into a probability distribution, allowing the model to assign a probability to each class and make a multi-class classification.") #5 points
...
print("The size of the first hidden layer is 300. None means the batch size can vary.\
The total number of parameters of the first hidden layer is 235,500, which refers to the number of weights and biases.") #8 points
model.summary()
...
print("An epoch is one complete pass through the entire training dataset during the training process.") #6 points
...
for i in range(10):
    plt.subplot(5, 2, i + 1)
    plt.text(30, 10, 'Predicted label: ' + str(np.argmax(y_pred[i])), fontsize=8, color='black') #6 points
